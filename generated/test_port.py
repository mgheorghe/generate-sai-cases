...
class TestSaiPort:
    ...
    @pytest.mark.dependency(
        depends=['test_sai_port_attr_path_tracing_timestamp_type_set']
    )
    def test_sai_port_attr_path_tracing_timestamp_type_get(self, npu):
        ...
        r_value = results[0][0].value()
        print(r_value)
        assert r_value == 'SAI_PORT_PATH_TRACING_TIMESTAMP_TYPE_16_23', (
            'Get error, expected SAI_PORT_PATH_TRACING_TIMESTAMP_TYPE_16_23 but got %s'
            % r_value
        )
    ...
    def test_sai_port_attr_rx_snr_get(self, npu):
        commands = [
            {'name': 'port_1', 'op': 'get', 'attributes': ['SAI_PORT_ATTR_RX_SNR']}
        ]
        results = [*npu.process_commands(commands)]
        print('======= SAI commands RETURN values get =======')
        for command in results:
    ...
    def test_sai_port_attr_datapath_enable_get(self, npu):
        ...
        r_value = results[0][0].value()
        print(r_value)
        assert r_value == 'SAI_PORT_DATAPATH_ENABLE_PASS_THROUGH_MODE', (
            'Get error, expected SAI_PORT_DATAPATH_ENABLE_PASS_THROUGH_MODE but got %s'
            % r_value
        )
